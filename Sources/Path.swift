...
extension Pathish {
	...
	public func join<S>(_ addendum: S) -> Path where S: StringProtocol {
		Path(string: join_(prefix: string, appending: addendum))
	}
	 ...
	 */
	@inlinable
	public static func / <S>(lhs: Self, rhs: S) -> Path where S: StringProtocol {
		lhs.join(rhs)
	}
	 ...
	 */
	public func relative<P: Pathish>(to base: P) -> String {
...
private func join_<S>(prefix: String, appending: S) -> String where S: StringProtocol {
	join_(prefix: prefix, pathComponents: appending.split(separator: "/"))
}
...
public struct DynamicPath: Pathish {
	...
	public subscript(dynamicMember addendum: String) -> DynamicPath {
		...
		DynamicPath(string: join_(prefix: string, appending: addendum))
	}
}
