...
class PathTests: XCTestCase {
	...
	func testFilesystemAttributes() throws {
		XCTAssert(Path(#filePath)!.isFile)
		XCTAssert(Path(#filePath)!.isReadable)
		XCTAssert(Path(#filePath)!.isWritable)
		XCTAssert(Path(#filePath)!.isDeletable)
		XCTAssert(Path(#filePath)!.parent.isDirectory)
	...
	func testDataExtensions() throws {
		let data = try Data(contentsOf: Path(#filePath)!)
		try Path.mktemp { tmpdir in
	...
	func testStringExtensions() throws {
		let string = try String(contentsOf: Path(#filePath)!)
		try Path.mktemp { tmpdir in
	...
	func testFileHandleExtensions() throws {
		_ = try FileHandle(forReadingAt: Path(#filePath)!)
		_ = try FileHandle(forWritingAt: Path(#filePath)!)
		_ = try FileHandle(forUpdatingAt: Path(#filePath)!)
	}
