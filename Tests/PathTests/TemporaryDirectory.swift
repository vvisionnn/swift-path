...
class TemporaryDirectory {
	let url: URL
	var path: DynamicPath { DynamicPath(Path(string: url.path)) }
	...
	init(appropriateFor: URL? = nil) throws {
		...
		self.url = try FileManager.default.url(for: .itemReplacementDirectory, in: .userDomainMask, appropriateFor: appropriate, create: true)
		#else
		...
		self.url = URL(fileURLWithPath: String(cString: template))
		#endif
	}
